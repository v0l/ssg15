...
function ShowGames(div){
	...
	getXHR('/data/rooms', true, function(data) {
		...
		for(var x=0;x<roomlist.roomlist.length;x++){
			...
			var row = "<tr><td>"+ room.roomId +"</td><td>" + room.data.level + "</td><td>" + room.stats.num_active_players + "/" + room.stats.num_players + "</td><td>" + s[room.data.status] + "</td><td><input type=\"button\" onclick=\"JoinGame(" + room.roomId + ")\" value=\"Join Game\"/></td></tr>"; 
			tbody += row;
		}
		e.innerHTML = tbody;
	});
